...
直接看重点的`worker_thread_`部分。该部分使用一个lambda表达式创建一个新的后台线程。该新线程会在一个死循环中等待interval的时间，**如果期间active_变为false，则退出循环，结束线程**；否则，调用回调函数 callback_fun执行任务。至于active_如何变为false，就要看析构函数了。
